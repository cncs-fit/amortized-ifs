...
SUMMARY_ROOT="outputs/oracle/ood_fractal_eval_n${NUM_SAMPLES}_chamfer010_p512_robust2048"
...
summarize_scenario() {
  ...
  "$PYTHON_BIN" scripts/summarize_oracle_quality_speed.py \
    ...
    --run model-10 "outputs/oracle/ood_${scenario}_model_s10_n${NUM_SAMPLES}_chamfer010_p512" \
    --run model-20 "outputs/oracle/ood_${scenario}_model_s20_n${NUM_SAMPLES}_chamfer010_p512" \
    ...
    --run random_r4-10 "outputs/oracle/ood_${scenario}_random_r4_s10_n${NUM_SAMPLES}_chamfer010_p512" \
    --run random_r4-20 "outputs/oracle/ood_${scenario}_random_r4_s20_n${NUM_SAMPLES}_chamfer010_p512" \
    --run random_r4-30 "outputs/oracle/ood_${scenario}_random_r4_s30_n${NUM_SAMPLES}_chamfer010_p512" \
    --run random_r4-60 "outputs/oracle/ood_${scenario}_random_r4_s60_n${NUM_SAMPLES}_chamfer010_p512" \
    --fidelity-name "$FIDELITY_NAME" \
    --output-dir "$out_dir"

  "$PYTHON_BIN" scripts/analyze_oracle_pareto.py \
    --summary-csv "$out_dir/summary.csv" \
    --output-dir "$out_dir/pareto"

  "$PYTHON_BIN" scripts/compare_oracle_per_sample_wins.py \
    --run model-30 "outputs/oracle/ood_${scenario}_model_s30_n${NUM_SAMPLES}_chamfer010_p512" \
    --run random_r4-30 "outputs/oracle/ood_${scenario}_random_r4_s30_n${NUM_SAMPLES}_chamfer010_p512" \
    --run random_r4-60 "outputs/oracle/ood_${scenario}_random_r4_s60_n${NUM_SAMPLES}_chamfer010_p512" \
    --comparison model30_vs_random30 model-30 random_r4-30 \
    --comparison model30_vs_random60 model-30 random_r4-60 \
    --fidelity-name "$FIDELITY_NAME" \
    --output-csv "$out_dir/per_sample_wins.csv"
}
...
run_scenario() {
  ...
  for steps in 0 10 20 30; do
    run_oracle "$scenario" "$scale_low" "$scale_high" "$fixed_point_range" model model 1 "$steps" "$seed"
  done
  for steps in 0 10 20 30 60; do
    run_oracle "$scenario" "$scale_low" "$scale_high" "$fixed_point_range" random_r4 random 4 "$steps" "$seed"
  done
  summarize_scenario "$scenario"
}

echo "OOD fractal full evaluation"
